...
func NotifyCronTaskFailure(taskName string, errorMsg string) {
	subject := "Scheduled Task Failed"
	content := fmt.Sprintf("Task: %s\nError: %s\nTime: %s", taskName, errorMsg, time.Now().Format("2006-01-02 15:04:05"))
	NotifyRootUser(dto.NotifyTypeCronTaskFailed, subject, content)
}
