...
export const mergeAdminConfig = (savedConfig) => {
  ...
  for (const [sectionKey, sectionConfig] of Object.entries(savedConfig)) {
    ...
    // Merge while preserving default values for new fields not in saved config
    merged[sectionKey] = { ...merged[sectionKey], ...sectionConfig };
  }

  // Ensure all default admin fields exist (for newly added features)
  if (merged.admin) {
    merged.admin = { ...DEFAULT_ADMIN_CONFIG.admin, ...merged.admin };
  }
